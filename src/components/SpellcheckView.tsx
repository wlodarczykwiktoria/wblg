...
import type { SpellcheckAnswerRequest, SpellcheckRiddle } from '../api/model.ts';
...
export const SpellcheckView: React.FC<Props> = ({ apiClient, type, language, bookId = 0, chapter = 0, onFinishLevel }) => {
  const [loading, setLoading] = useState(false);
  const [riddles, setRiddles] = useState<SpellcheckRiddle[]>([]);
  ...
  const startGame = useCallback(async () => {
    ...
    const response = await apiClient.startSpellcheckGame(bookId, chapter);
  ...
  const finishGame = useCallback(async () => {
    ...
    await finishGameWithResults({
      ...
      submitAnswers: () => apiClient.submitSpellcheckAnswers(payload),
  ...
  return (
    <GameRoundLayout
      ...
      heading={t.spellcheckHeading}
      instructions={t.spellcheckInstructions}
    ...
    >
      <SpellcheckGame riddle={riddle} selectedWordIds={selectedWordIds} onToggleWord={handleToggleWord} />
    </GameRoundLayout>
  );
};
